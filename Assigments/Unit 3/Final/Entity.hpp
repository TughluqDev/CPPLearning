...
//bar color changes as hp drops: green is fine, yellow is low, red is almost dead
inline string hpColor(int hp, int maxHp) {
    if (maxHp <= 0) return Color::RED;
...
public:
    Entity(const string& n, int hp_, int atk, int def, int spd, int crit = 10, Element elem = Element::None):
    name(n), hp(hp_), maxHp(hp_), attackDmg(atk), defense(def), speed(spd), critChance(crit), element(elem) {}
    ...
    int attackTarget(Entity& target) {
        ...
        if (crit) dmg = max(1, static_cast<int>(dmg * 1.5f));
        ...
        if (mod > 1.0f) cout << " " << Color::GREEN  << "Super effective!" << Color::RESET;
        if (mod < 1.0f) cout << " " << Color::BLUE   << "Not very effective" << Color::RESET;
        if (crit)       cout << " " << Color::BOLD << Color::YELLOW << "CRIT!" << Color::RESET;
        cout << "\n";
    ...
    bool useItemOn(int itemIndex, Entity& target) {
        if (itemIndex < 0 || itemIndex >= (int)inventory.size()) return false;
        ...
        if (amount > 0) cout << " (" << Color::GREEN << "+" << amount << " HP" << Color::RESET << ")";
        cout << "!\n";
